...
from astronet.tf_slim import learning
from astronet.tf_slim.layers import layers
...
class MultiplyGradientsTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self._grad_vec = np.array([1., 2., 3.])
        self._multiplied_grad_vec = np.multiply(
            self._grad_vec, self._multiplier)
...
class CreateTrainOpTest(test.TestCase):
    ...
    def testUseUpdateOps(self):
        with ops.Graph().as_default():
            ...
            moving_mean = variables_lib2.get_variables_by_name('moving_mean')[
                0]
...
class TrainTest(test.TestCase):
    ...
    def testTrainWithNoneAsInitWhenUsingVarsRaisesError(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.assertRaises(RuntimeError):
                learning.train(train_op, logdir, init_op=None,
                               number_of_steps=300)
    ...
    def testTrainWithEpochLimit(self):
        ...
        with ops.Graph().as_default():
            ...
            tf_inputs = tf.constant(self._inputs, dtype=tf.float32)
            tf_labels = tf.constant(self._labels, dtype=tf.float32)
            tf_inputs_limited = input_lib.limit_epochs(
                tf_inputs, num_epochs=300)
            tf_labels_limited = input_lib.limit_epochs(
                tf_labels, num_epochs=300)
        ...
        self.assertTrue(os.path.isfile(
            '{}/model.ckpt-300.index'.format(logdir)))
        self.assertTrue(
