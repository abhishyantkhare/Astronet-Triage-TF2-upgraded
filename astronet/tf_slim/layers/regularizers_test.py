...
class RegularizerTest(test.TestCase):
    ...
    def testL1L2RegularizerWithScope(self):
        with self.cached_session():
            ...
            with ops.name_scope('foo'):
                loss = regularizers.l1_l2_regularizer(
                    1.0, 1.0, scope='l1_l2')(tensor)
    ...
    def test_apply_regularization(self):
        def dummy_regularizer(x): return math_ops.reduce_sum(2 * x)
        array_weights_list = [[1.5], [2, 3, 4.2], [10, 42, 666.6]]
        tensor_weights_list = [constant_op.constant(
            x) for x in array_weights_list]
    ...
    def test_apply_regularization_invalid_regularizer(self):
        def non_scalar_regularizer(x): return array_ops.tile(x, [2])
        tensor_weights_list = [
