...
from astronet.tf_slim import layers as layers_lib
from astronet.tf_slim.layers import layers as _layers
from astronet.tf_slim.layers import regularizers
from astronet.tf_slim.ops import arg_scope as arg_scope_lib
...
class AvgPool2DTest(test.TestCase):
    ...
    def testCollectOutputs(self):
        ...
        output = _layers.avg_pool2d(
            images, [3, 3], outputs_collections='outputs')
...
class AvgPool3DTest(test.TestCase):
    ...
    def testCreateAvgPoolStrideWithSamePadding(self):
        ...
        images = random_ops.random_uniform(
            (5, depth, height, width, 3), seed=1)
        output = _layers.avg_pool3d(
            images, [3, 3, 3], stride=1, padding='SAME')
        self.assertListEqual(output.get_shape().as_list(),
...
class PoolTest(test.TestCase):
    ...
    def testAvgPoolWithStride(self):
        height, width = 5, 8
        images = random_ops.random_uniform((5, height, width, 3), seed=1)
        output = _layers.pool(images, [2, 3], stride=[
                              1, 2], pooling_type='AVG')
        ...
        with self.cached_session():
            for d in dims:
                ...
                self.assertListEqual(
                    biases.get_shape().as_list(), [input_shape[-1]])
...
class ConvolutionTest(test.TestCase):
    ...
    def testCreateConv(self):
        ...
        with self.cached_session():
            images = np.random.uniform(
                size=(5, height, width, 4)).astype(np.float32)
    ...
    def testCreateConvNCHW(self):
        ...
        with self.cached_session():
            images = np.random.uniform(
                size=(5, 4, height, width)).astype(np.float32)
    ...
    def testCreateConvWithTensorShape(self):
        ...
        with self.cached_session():
            images = random_ops.random_uniform((5, height, width, 3), seed=1)
            output = layers_lib.convolution2d(
                images, 32, images.get_shape()[1:3])
            self.assertEqual(output.op.name, 'Conv/Relu')
            self.assertListEqual(output.get_shape().as_list(), [
                                 5, height, width, 32])
    ...
    def testCreateConvWithoutActivation(self):
        ...
        with self.cached_session():
            images = random_ops.random_uniform((5, height, width, 3), seed=1)
            output = layers_lib.convolution2d(
                images, 32, [3, 3], activation_fn=None)
    ...
    def testCreateConvWithWD(self):
        ...
        with self.cached_session() as sess:
            ...
            l2_loss = nn_ops.l2_loss(
                variables.get_variables_by_name('weights')[0])
            wd = ops.get_collection(ops.GraphKeys.REGULARIZATION_LOSSES)[0]
            self.assertEqual(
                wd.op.name, 'Conv/kernel/Regularizer/l2_regularizer')
    ...
    def testReuseVars(self):
        ...
        with self.cached_session():
            ...
            layers_lib.convolution2d(
                images, 32, [3, 3], scope='conv1', reuse=True)
            self.assertEqual(len(variables.get_variables()), 2)
    ...
    def testDynamicOutputSizeWithRateOneValidPadding(self):
        ...
        with self.cached_session():
            ...
            eval_output = output.eval(
                {images: np.zeros(input_size, np.float32)})
            self.assertListEqual(list(eval_output.shape),
                                 expected_size_dynamic)
    ...
    def testDynamicOutputSizeWithRateOneValidPaddingNCHW(self):
        if test.is_gpu_available(cuda_only=True):
            ...
            with self.session(use_gpu=True):
                ...
                self.assertEqual(output.op.name, 'Conv/Relu')
                self.assertListEqual(
                    output.get_shape().as_list(), expected_size)
                eval_output = output.eval(
                    {images: np.zeros(input_size, np.float32)})
                self.assertListEqual(
                    list(eval_output.shape), expected_size_dynamic)
        ...
        for trainable in [True, False]:
            with ops.Graph().as_default():
                ...
                for model_variable in model_variables:
                    self.assertEqual(
                        trainable, model_variable in trainable_variables)
    ...
    def testInvalidDataFormat(self):
        ...
        with self.cached_session():
            ...
            with self.assertRaisesRegexp(
                ...
                _layers.convolution2d_transpose(
    ...
    def testOutputSizeWithStrideTwoValidPaddingNCHW(self):
        if test.is_gpu_available(cuda_only=True):
            with self.session(use_gpu=True) as sess:
                ...
                self.assertEqual(output.op.name, 'Conv2d_transpose/Relu')
                self.assertListEqual(
                    list(output.get_shape().as_list()), expected_size)
...
class ConvolutionInPlaneTest(test.TestCase):
    ...
    def testHorzConvWithBlankImageAndPlaceholder(self):
        image = array_ops.placeholder(
            dtypes.float32, shape=(None, None, None, 1))
        horz_gradients = layers_lib.conv2d_in_plane(
            image,
            weights_initializer=init_ops.constant_initializer([1, -1]),
            kernel_size=[1, 2],
...
class DenseToSparseTest(test.TestCase):
    ...
    def testDenseFromConstantToSparse(self):
        expected_constant = np.reshape(
            np.arange(24, dtype=np.int64), (3, 4, 2))
...
class FlattenTest(test.TestCase):
    ...
    def testFlattenBatchSize(self):
        ...
        with self.cached_session() as sess:
            ...
            inputs = array_ops.placeholder(
                dtypes.int32, (None, height, width, 3))
            output = _layers.flatten(inputs)
    ...
    def testUnknownDims(self):
        ...
        with self.cached_session() as sess:
            ...
            inputs = array_ops.placeholder(
                dtypes.int32, (None, None, None, None))
            output = _layers.flatten(inputs)
            output = sess.run(output, {inputs: images.eval()})
            self.assertEqual(output.size, images.get_shape().num_elements())
            self.assertEqual(output.shape[0], images.get_shape()[0])
...
class PartialFlattenTest(test.TestCase):
    ...
    def testSparsePartialFlatten(self):
        ...
        for new_rank in [1, 2, 3]:
            expected_shape = (shape[:new_rank - 1] +
            ...
            np.testing.assert_array_equal(
                expected_shape, flattened.dense_shape)
    ...
    def testIncompleteShape(self):
        ...
        self.assertEqual([2, None, 4, None, 30],
                         flattened5.get_shape().as_list())
...
class FCTest(test.TestCase):
    ...
    def testCreateFC(self):
        ...
        for layer_fn in (_layers.fully_connected, layers_lib.relu):
            with ops.Graph().as_default() as g, self.session(g):
                ...
                weights = variables.get_variables_by_name('weights')[0]
                self.assertListEqual(
                    weights.get_shape().as_list(), [3 * 3 * 3, 32])
                biases = variables.get_variables_by_name('biases')[0]
                self.assertListEqual(biases.get_shape().as_list(), [32])
    ...
    def testNonReuseVars(self):
        ...
        with self.cached_session():
            ...
            self.assertEqual(
                len(variables.get_variables('fully_connected')), 2)
            _layers.fully_connected(inputs, 32)
            self.assertEqual(
                len(variables.get_variables('fully_connected')), 4)

    def testReuseWithRegularizer(self):
        height, width = 3, 3

        def regularizer(x): return math_ops.reduce_sum(x) * 1e-3
        inputs = random_ops.random_uniform((5, height * width * 3), seed=1)
        ...
        with variable_scope.variable_scope('outer', reuse=False):
            _layers.fully_connected(
                inputs, 32, weights_regularizer=regularizer)
            self.assertEqual(
    ...
    def testCreateFCWithWD(self):
        ...
        with self.cached_session() as sess:
            ...
            weight_decay = regularizers.l2_regularizer(0.01)
            _layers.fully_connected(
                inputs, 32, weights_regularizer=weight_decay)
            wd = ops.get_collection(ops.GraphKeys.REGULARIZATION_LOSSES)[0]
...
class BatchNormTest(test.TestCase):
    ...
    def _testCreateOpBetaRegularizer(self, fused=True):
        height, width = 3, 3
        with self.cached_session():
            def reg(x): return 0.1 * math_ops.reduce_sum(x)
            images = np.random.uniform(size=(5, height, width, 3)).astype('f')
            _layers.batch_norm(images, param_regularizers={
                               'beta': reg}, fused=fused)
            self.assertEqual(
                len(ops.get_collection(ops.GraphKeys.REGULARIZATION_LOSSES)), 1)
            beta_decay = ops.get_collection(
                ops.GraphKeys.REGULARIZATION_LOSSES)[0]
            self.assertEqual(beta_decay.op.name,
    ...
    def _testCreateOpGammaRegularizer(self, fused=True):
        ...
        with self.cached_session():
            ...
            self.assertEqual(
                len(ops.get_collection(ops.GraphKeys.REGULARIZATION_LOSSES)), 1)
            gamma_decay = ops.get_collection(
                ops.GraphKeys.REGULARIZATION_LOSSES)[0]
            self.assertEqual(gamma_decay.op.name,
    ...
    def testMovingAverageVariablesZeroDebias(self):
        ...
        with self.cached_session():
            ...
            self.assertEqual(moving_variance.op.name,
                             'BatchNorm/moving_variance')
            self.assertEqual(
                biased.op.name, 'BatchNorm/BatchNorm/moving_mean/biased')
    ...
    def testReuseVariables(self):
        ...
        with self.cached_session():
            ...
            moving_variance = variables.get_variables_by_name(
                'moving_variance')
    ...
    def testZeroDebiasMovingMean(self):
        ...
        output = _layers.batch_norm(
            ...
            fused=False)
        moving_mean = variables.get_variables_by_name(
    ...
    def _testTrainMovingVars(self, fused, data_format='NHWC'):
        ...
        with self.session(use_gpu=use_gpu) as sess:
            ...
            for _ in range(10):
                np_output, new_images_gradients = sess.run(
                    [output, images_gradients])
                ...
                max_diff = np.max(
                    np.abs(images_gradients_value - new_images_gradients))
    ...
    def testBatchNormBeta(self):
        # Test case for 11673
        with self.cached_session() as sess:
            a_32 = array_ops.placeholder(
                dtypes.float32, shape=(10, 10, 10, 10))
            _layers.batch_norm(
                a_32, center=False, data_format='NCHW', zero_debias_moving_mean=True)
            a_16 = array_ops.placeholder(
                dtypes.float16, shape=(10, 10, 10, 10))
            _layers.batch_norm(
    ...
    def testAdjustmentCreated(self):
        ...
        def _create_adjustment(shape):
            adjustments = [array_ops.ones(
                shape[-1:]), array_ops.zeros(shape[-1:])]
    ...
    def doOutputTest(self,
        ...
        for mu in [0.0, 1e2 if dtype != dtypes.float16 else 1e1]:
            for sigma in sigma_list:
                expected_var = expected_var_uncorrected / \
                    (1.0 + eps / sigma**2)
                ...
                with ops.Graph().as_default() as g:
                    with self.session(graph=g) as sess:
                        ...
                        moments_axis = tuple(
                            range(begin_norm_axis, len(input_shape)))
                        with variable_scope.variable_scope('LN', reuse=True):
                            beta_var = variable_scope.get_variable(
                                'beta', dtype=dtype)
                            gamma_var = variable_scope.get_variable(
                                'gamma', dtype=dtype)
                        outputs, beta, gamma = sess.run(
                            (output_t, beta_var, gamma_var))
...
class ImagesToSequenceTest(test.TestCase):
    ...
    def testImagesToSequenceDims(self):
        height, width = 7, 11
        images = np.random.uniform(
            size=(2, height, width, 5)).astype(np.float32)
    ...
    def testImagesToSequenceNCHW(self):
        height, width = 7, 11
        images = np.random.uniform(
            size=(2, 5, height, width)).astype(np.float32)
...
class MaxPool2DTest(test.TestCase):
    ...
    def testCreateMaxPool(self):
        height, width = 3, 6
        images = np.random.uniform(
            size=(5, height, width, 3)).astype(np.float32)
    ...
    def testCreateMaxPoolNCHW(self):
        height, width = 3, 6
        images = np.random.uniform(
            size=(5, 3, height, width)).astype(np.float32)
...
class OneHotEncodingTest(test.TestCase):
    ...
    def testOneHotEncoding(self):
        with self.cached_session():
            labels = constant_op.constant([0, 1, 2])
            one_hot_labels = constant_op.constant(
                [[1, 0, 0], [0, 1, 0], [0, 0, 1]])
            output = _layers.one_hot_encoding(labels, num_classes=3)
...
class SeparableConv2dTest(test.TestCase):
    ...
    def testCreateConvCreatesWeightsAndBiasesVars(self):
        ...
        images = random_ops.random_uniform((5, height, width, 3), seed=1)
        with self.cached_session():
            self.assertFalse(variables.get_variables(
                'conv1/depthwise_weights'))
            self.assertFalse(variables.get_variables(
                'conv1/pointwise_weights'))
            self.assertFalse(variables.get_variables('conv1/biases'))
    ...
    def testCreateAtrousConvCreatesWeightsAndBiasesVars(self):
        ...
        with self.cached_session():
            ...
            layers_lib.separable_conv2d(
                images, 32, [3, 3], 4, rate=2, scope='conv1')
            self.assertTrue(variables.get_variables('conv1/depthwise_weights'))
            self.assertTrue(variables.get_variables('conv1/pointwise_weights'))
            self.assertTrue(variables.get_variables('conv1/biases'))
    ...
    def testCreateConvWithWeightDecay(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertEqual(
                len(ops.get_collection(ops.GraphKeys.REGULARIZATION_LOSSES)), 2)
            weight_decay = ops.get_collection(
                ops.GraphKeys.REGULARIZATION_LOSSES)[0]
            self.assertEqual(
            ...
            weight_decay = ops.get_collection(
                ops.GraphKeys.REGULARIZATION_LOSSES)[1]
            self.assertEqual(
                weight_decay.op.name,
            ...
            weight_decay = ops.get_collection(
                ops.GraphKeys.REGULARIZATION_LOSSES)[2]
            sess.run(variables_lib.global_variables_initializer())
            depth_weight_two = sess.run(weight_decay)
            weight_decay = ops.get_collection(
                ops.GraphKeys.REGULARIZATION_LOSSES)[3]
        ...
        for num_filters, correct_output_filters in zip((None, 5), (6, 5)):
            with self.cached_session():
                ...
                output = layers_lib.separable_conv2d(
                    ...
                    data_format='NCHW')
                self.assertListEqual(output.get_shape().as_list(), [
                    batch, correct_output_filters, (height -
                                                    kernel_dim + 1) // stride,
...
class ScaleGradientTests(test.TestCase):
    ...
    def testBasic(self):
        with self.cached_session():
            ...
            g_x, = gradients_impl.gradients(
                y, [x], [np.array([3], np.float32)])
...
class SpatialSoftmaxTests(test.TestCase):
    ...
    def testSpatialSoftmaxShape(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertAllEqual(
                keypoints.shape, (batch_shape[0], batch_shape[3] * 2))
    ...
    def testSpatialSoftmaxShapeNCHW(self):
        ...
        with self.cached_session() as sess:
            ...
            keypoints = sess.run(spatial_softmax, feed_dict)
            self.assertAllEqual(
                keypoints.shape, (batch_shape[0], batch_shape[1] * 2))
    ...
    def testMaxActivationsAtEdges(self):
        ...
        edges = [(0, 0), (0, width - 1), (height - 1, 0),
                 (height - 1, width - 1)]
    ...
    def testSpatialSoftmaxVariableSized(self):
        ...
        features = array_ops.placeholder(
            dtypes.float32, shape=variable_sized_shape)
        spatial_softmax = _layers.spatial_softmax(features)
    ...
    def testStackFullyConnected(self):
        ...
        with self.cached_session():
            ...
            output = _layers.stack(
                images, _layers.fully_connected, [10, 20, 30])
...
class PoincareNormalizeTest(test.TestCase):
    ...
    def _PoincareNormalize(self, x, dim, epsilon=1e-5):
        if isinstance(dim, list):
            ...
            norm = np.expand_dims(np.apply_along_axis(
                np.linalg.norm, dim, x), dim)
            norm_x = ((1. - epsilon) * x) / norm
...
class LegacyFullyConnectedTest(test.TestCase):
    ...
    def _fully_connected_basic_use(self, x, num_output_units, expected_shape):
        ...
        with session.Session() as sess:
            ...
            variables_lib.global_variables_initializer().run()
            out_value, shape_value = sess.run(
                [output, array_ops.shape(output)])
    ...
    def test_variable_reuse_with_scope(self):
        ...
        with session.Session() as sess:
            variables_lib.global_variables_initializer().run()
            out_value1, out_value2, out_value3 = sess.run(
    ...
    def test_regularizer(self):
        ...
        _layers.legacy_fully_connected(
            self.input, 2, weight_regularizer=test_fn)
    ...
    def test_empty_x_results_in_empty_output(self):
        ...
        with self.cached_session():
            ...
            y = _layers.legacy_fully_connected(
                x, 2, activation_fn=nn_ops.softmax)
            variables_lib.global_variables_initializer().run()
