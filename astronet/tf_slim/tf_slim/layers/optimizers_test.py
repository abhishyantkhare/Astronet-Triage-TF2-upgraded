...
from astronet.tf_slim.tf_slim.layers import optimizers as optimizers_lib
...
class OptimizersTest(test.TestCase):
    ...
    def testSGDOptimizer(self):
        optimizers = [
            ...
            lambda lr: gradient_descent.GradientDescentOptimizer(
                learning_rate=lr),
            "Momentum"
        ]
...
class AdaptiveClipping(test.TestCase):
    ...
    def testClip(self):
        with self.cached_session() as session:
            spike = 1000.
            multiplier = array_ops.placeholder(
                dtypes.float32, [], "multiplier")
            ...
            for i in range(20):
                ...
                if i > 3:
                    self.assertAllClose(
                        np.ones(clipped_grad.shape) * scale, clipped_grad)
            ...
            self.assertAllClose(np.ones(clipped_grad.shape)
                                * spike, clipped_grad)
