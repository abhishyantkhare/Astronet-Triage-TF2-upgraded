...
from astronet.tf_slim.tf_slim.layers import normalization
from astronet.tf_slim.tf_slim.ops import variables as contrib_variables
...
class GroupNormTest(test.TestCase):
    ...
    def testNotMutuallyExclusiveAxis(self):
        ...
        with self.assertRaisesRegexp(ValueError, 'mutually exclusive'):
            normalization.group_norm(
                inputs, channels_axis=-2, reduction_axes=[-2])
    ...
    def testCreateOp(self):
        height, width, groups = 3, 3, 4
        images = random_ops.random_uniform(
            (5, height, width, 2*groups), seed=1)
    ...
    def testCreateVariables_NCHW(self):
        height, width, groups = 3, 3, 4
        images = random_ops.random_uniform(
            (5, 2*groups, height, width), seed=1)
        normalization.group_norm(images, groups=4,
    ...
    def doOutputTest(self,
        ...
        for mu in mu_tuple:
            for sigma in sigma_tuple:
                ...
                inputs = random_ops.random_normal(
                    input_shape, seed=0) * sigma + mu
    ...
    def testOutputSmallInput4D_NHWC(self):
        ...
        self.doOutputTest(input_shape, channels_axis=-
                          1, reduction_axes=[-3, -2])
        # Specify axes with positive values.
