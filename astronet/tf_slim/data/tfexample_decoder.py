...
class Tensor(ItemHandler):
    ...
    def tensors_to_item(self, keys_to_tensors):
        ...
        if isinstance(tensor, sparse_tensor.SparseTensor):
            ...
            tensor = sparse_ops.sparse_tensor_to_dense(
...
class LookupTensor(Tensor):
    ...
    def tensors_to_item(self, keys_to_tensors):
        unmapped_tensor = super(
            LookupTensor, self).tensors_to_item(keys_to_tensors)
...
class BackupHandler(ItemHandler):
    ...
    def tensors_to_item(self, keys_to_tensors):
        ...
        return control_flow_ops.cond(
            pred=math_ops.equal(math_ops.reduce_prod(
                array_ops.shape(item)), 0),
