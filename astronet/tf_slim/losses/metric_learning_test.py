...
from astronet.tf_slim.losses import metric_learning
...
class ContrastiveLossTest(test.TestCase):
    ...
    def testContrastive(self):
        with self.cached_session():
            ...
            embeddings_anchor = np.random.rand(
                num_data, feat_dim).astype(np.float32)
            embeddings_positive = np.random.rand(num_data, feat_dim).astype(
                np.float32)
            labels = np.random.randint(
                0, 2, size=(num_data,)).astype(np.float32)
    ...
    def loss_augmented_fit(self, feat, y, loss_mult):
        ...
        while k < self.n_clusters:
            ...
            for i in candidate_ids:
                ...
                subset.append(i)
                marginal_cost = -1.0 * \
                    np.sum(np.min(pdists[:, subset], axis=1))
        ...
        self.score_ = np.float32(-1.0) * \
            self._get_facility_distance(pdists, subset)
    ...
    def _augmented_update_medoid_ics_in_place(self, pdists, y_gt, cluster_ics,
        ...
        for cluster_idx in range(self.n_clusters):
            ...
            for i in range(y_pred.size):
                ...
                medoid_ics_i = medoid_ics[:cluster_idx] + \
                    medoid_ics[cluster_idx + 1:]
...
class ClusterLossTest(test.TestCase):
    ...
    def testClusteringLossPAMOff(self):
        ...
        with self.cached_session():
            ...
            embeddings, labels = self._genClusters(
                n_samples=128, n_clusters=64)
