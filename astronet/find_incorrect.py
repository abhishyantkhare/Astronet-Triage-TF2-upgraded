...
def predict(model_dir, config, model_class, y_pred=defaultdict(list), true_disp=defaultdict(list)):
    ...
    g = tf.Graph()
    with g.as_default():
        example_placeholder = tf.compat.v1.placeholder(tf.string, shape=[])
        parsed_features = tf.io.parse_single_example(
            serialized=example_placeholder,
            features={
    ...
    with tf.compat.v1.Session(graph=g) as sess:
        ...
        tf.compat.v1.logging.info("Successfully loaded checkpoint %s at global step %d.",
                        checkpoint_file, sess.run(model.global_step))

        for filename in filenames:
            print(filename)
            for i, serialized_example in enumerate(tf.compat.v1.python_io.tf_record_iterator(filename)):
                prediction = sess.run(
                    model.predictions,
                ...
                if i % 100 == 0:
                    tf.compat.v1.logging.info("Processed %d items in file %s", i, filename)
...
def main(_):
    ...
    for tce in y_pred:
        ...
        if cnt % 100 == 0:
            tf.compat.v1.logging.info("Averaging %d of %d", cnt, num_tces)
...
if __name__ == "__main__":
  ...
  test_name = 'test*'
  filenames = tf.io.gfile.glob(os.path.join(FLAGS.tfrecord_dir, test_name))
