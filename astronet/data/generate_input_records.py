...
def _process_tce(tce):
  ...
  try:
    ...
    # secondary_view = preprocess.secondary_view(time, flux, tce.Period, tce.Duration)
      ...
      tf.compat.v1.logging.info('Too many invalid values in TIC %s', tce.tic_id)
...
def _process_file_shard(tce_table, file_name):
  ...
  tf.compat.v1.logging.info("%s: Processing %d items in shard %s", process_name,
  ...
  with tf.io.TFRecordWriter(file_name) as writer:
    ...
    for _, tce in tce_table.iterrows():
      ...
      if not num_processed % 10:
        tf.compat.v1.logging.info("%s: Processed %d/%d items in shard %s", process_name,
                        num_processed, shard_size, shard_name)

  tf.compat.v1.logging.info("%s: Wrote %d/%d items in shard %s. %d skipped.", process_name, num_processed, shard_size,
...
def create_input_list():
    ...
    tf.compat.v1.logging.info("Read TCE CSV file with %d rows.", len(tce_table))

    # replace "IS" and "V" labels with "J" for junk
    tce_table['Disposition'] = tce_table['Disposition'].replace({'IS': 'J', 'V': 'J'})
    num_tces = len(tce_table)
    tf.compat.v1.logging.info("Filtered to %d TCEs", num_tces)
...
def make_eval_set(argv):
    ...
    num_tces = len(tce_table)
    tf.compat.v1.logging.info('Read in %s TCEs', num_tces)

    # Randomly shuffle the TCE table.
    np.random.seed(123)
    tce_table = tce_table.iloc[np.random.permutation(num_tces)]
    tf.compat.v1.logging.info("Randomly shuffled TCEs.")
    ...
    tf.compat.v1.logging.info("Launching %d subprocesses for %d total file shards",
    ...
    tf.compat.v1.logging.info("Finished processing %d total file shards", num_file_shards)
...
def main(argv):
  ...
  if FLAGS.clean:
      tf.compat.v1.logging.info("Excluding S/N < %s", FLAGS.threshold)
...
if __name__ == "__main__":
  ...
  if FLAGS.make_test_set:
      tf.compat.v1.app.run(main=make_eval_set, argv=[sys.argv[0]] + unparsed)
  else:
